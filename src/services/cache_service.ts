...
export class CacheService {
  ...
  async set<T>(key: string, value: T, ttl?: number): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('Cache entry set', { key: cacheKey, ttl: expirationTtl });
      
      // Record cache metrics
      this.metricsService.recordPerformance('cache-set', duration, true, 'cache-service', { key, ttl: expirationTtl });
    } catch (error) {
      const duration = Date.now() - startTime;
      this.logger.logCacheOperation('set', key, 'cache-service', duration);
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    try {
      ...
      this.metricsService.recordPerformance('cache-get', duration, true, 'cache-service', { key, result: 'hit' });
      return entry.data;
    } catch (error) {
      const duration = Date.now() - startTime;
      this.logger.logCacheOperation('miss', key, 'cache-service', duration);
  ...
  async cacheRPCResponse(chainId: string, method: string, params: any[], response: any): Promise<void> {
    ...
    const cacheKey = `rpc:${chainId}:${method}:${JSON.stringify(params)}`;
